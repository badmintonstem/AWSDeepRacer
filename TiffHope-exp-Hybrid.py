...
def reward_function(params):
    ...
    D_HEADING    = 10.0             # degrees at which heading factor hits zero (pre-clamp)
    ...
    # Curvature-speed
    CURV_LOOKAHEAD = 3              # use next 2 segments to estimate turn angle
    V_MIN        = 0.5              # m/s in tight turns
    V_MAX        = 4.0              # m/s on straights (match your action space)
    ALPHA_SPEED  = 0.10             # curvature → target speed sensitivity Larger number will slow down more (deg^-1)
    SPEED_FLOOR  = 0.3              # min speed factor so we never zero out reward - Lower number will penalise more if going too quick
    # Smoothness
    ...
    def _index(i):                   # wrap waypoint index
        n = len(waypoints)
        return i % n
